...
export default async function AdminRequests(props: { searchParams: Promise<{ q?: string; guild?: string }> }) {
  ...
  try {
      ...
      const rejectPresets = presetsRes.rows.length > 0 ? ((presetsRes.rows[0] as any).value || "").split(',').map((s: string) => s.trim()).filter(Boolean) : [];
      ...
      return (
          <div className="p-10 bg-error/10 text-error rounded-3xl">
              <h1 className="text-xl font-bold">Audit Page Error</h1>
