...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      // Assign to an open batch (matching bot logic)
    ...
    // --- PHASE 2: DISCORD SYNC (NON-BLOCKING) ---
    try {
        // Cleanup old message if exists
        ...
        if (action === "approve") {
            ...
            let batchIdRes = await execute("SELECT batch_id FROM batch_requests WHERE id = ?", [requestId]);
            const batchId = (batchIdRes.rows[0] as any).batch_id;
            ...
            if ((countRes.rows[0] as any).count >= 8) {
                ...
                if (relId) {
                    ...
                    const list = reqs.rows.map((r: any, i) => `**${i+1}.** ${r.username} — ID: \`${r.vrfs_id}\` (${r.type})`).join('\n');
                    await sendDiscordMessage(relId, { 
                        embeds: [{
        ...
        console.error("Discord operation failed but DB was updated:", discordErr);
    }
